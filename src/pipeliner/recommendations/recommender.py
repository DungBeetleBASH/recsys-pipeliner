...
class ItemBasedRecommender(BaseEstimator):
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if isinstance(X, list) or isinstance(X, tuple):
            ...
            if self.sort is not None:
                self.sort_mat = pd.DataFrame(
                    self.user_item.aggregate(self.sort, axis=1), columns=["ranking"]
                )
        else:
            self.sim_mat = X
    ...
    def _retrieve_preds(self, item):
        ...
        if self.sort_mat is None:
            ...
            pred = pred.join(self.sort_mat).sort_values(
                by=["similarity", "ranking"], ascending=self.ascending
            )
    ...
    def predict_proba(self, X):
        ...
        for item in X:
            try:
                ...
                res.append(
                    list(
                        zip(pred.index[: self.num_items], pred.values[: self.num_items])
                    )
                )
            except:
...
class CrossSimilarityRecommender(MultiOutputClassifier):
    ...
    def predict(self, X):
        ...
        if self.selection == "ranking" and self.rank_mat is None:
            raise RuntimeError(
                "No Ranking Matrix provided, but ranking option choosen!"
            )
