...
def test_EvaluationDataset_leave_one_out(mocker, fx_user_item_ratings_toy_np):
    mock_randint = mocker.patch("recsys_pipeliner.evaluation.dataset.np.random.randint")
...
def test_EvaluationDataset_leave_one_out_random_seed(
    mocker, fx_user_item_ratings_toy_np
):
    mock_RandomState = mocker.patch(
        "recsys_pipeliner.evaluation.dataset.np.random.RandomState"
    )
...
def test_EvaluationDataset_leave_one_out_min_ratings_error(fx_user_item_ratings_toy_np):
    with pytest.raises(ValueError, match="No users with enough ratings to split"):
        EvaluationDataset(
            fx_user_item_ratings_toy_np, min_user_ratings=20, min_item_ratings=20
        )
